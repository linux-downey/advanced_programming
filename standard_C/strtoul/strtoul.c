 ...
 *@param 第一个参数为需要转换的字符串,函数实现中自动忽略字符串前的空格
 *@param 第二个参数为第一个不能转换的字符指针,这个参数事实上是使用者传进去的指针,当字符串中有不能被转换的值时,这个指针的值就是第一个不能被转换字符的地址.
 *即使是遇到不能转换的字符,函数返回值也是已经转换的字符值.如果没有转换任何数字,返回0
 *@param 第三个参数为转换的进制,如10为转换成十进制,如果字符串以0x开头,自动转换成16进制,如果以字符串0开头,默认转换成8进制,填0默认为10进制.
 */
